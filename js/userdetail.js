...
onAuthStateChanged(auth, (user) => {
  ...
  get(child(dbref, `posts/`)).then((snapshot) => {
    ...
    snapshot.forEach((postRes) => {
      if (postRes.val().userPost !== uid || postRes.size == 0) {
      ...
      if(postRes.val().userPost == uid){
        ...
        dropDownShowComment.style.color = "black"
        ...
        dropDownHideComment.style.color = "black"
        ...
        get(child(dbref, `comments/${postRes.val().postKey}`)).then((res) => {
